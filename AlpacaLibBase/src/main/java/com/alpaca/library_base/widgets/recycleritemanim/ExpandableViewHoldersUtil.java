package com.alpaca.library_base.widgets.recycleritemanim;
...
public class ExpandableViewHoldersUtil {
    ...
    //类似于打开的方法
    private void closeHolder(final RecyclerView.ViewHolder holder, final View expandView, View changeView, final boolean animate) {
        if (animate) {

            expandView.setVisibility(View.GONE);
            ExpandableViewHoldersUtil.getInstance().rotateExpandIcon((ImageView) changeView, 0, 180);
            final Animator animator = ViewHolderAnimator.ofItemViewHeight(holder);
            expandView.setVisibility(View.VISIBLE);
    ...
    public class KeepOneHolder<VH extends RecyclerView.ViewHolder & Expandable> {
         ...
         * @param pos    下标
         */
        public void bind1(VH holder, int pos) {
//            if (explanedList.contains(pos + "")) {
            addPositionInExpaned(pos);
            ExpandableViewHoldersUtil.getInstance().openHolder(holder, holder.getExpandView(), false);
//            } else {
//                ExpandableViewHoldersUtil.getInstance().closeHolder(holder, holder.getExpandView(), false);
//            }
        ...
        public boolean toggle(VH holder) {
