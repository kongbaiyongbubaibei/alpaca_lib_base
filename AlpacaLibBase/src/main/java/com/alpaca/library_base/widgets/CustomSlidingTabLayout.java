...
import com.alpaca.library_base.utils.TextColorUtil;
...
/**
 * copy：SlidingTabLayout,在此源码基础上修改
 * 新增功能：
 * 1 新增选中字体大小和非选中字体大小设置
 * 2 新增tab背景样式设置
 * 3 新增tab间距设置
 */
public class CustomSlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private float mTextPaddingX, mTextPaddingY;
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextSelectBackground = ta.getResourceId(R.styleable.SlidingTabLayout_tl_textSelectBackground, 0);
        mTextUnselectColor = ta.getColor(R.styleable.SlidingTabLayout_tl_textUnselectColor, Color.parseColor("#AAffffff"));
        mTextUnSelectBackground = ta.getResourceId(R.styleable.SlidingTabLayout_tl_textUnSelectBackground, 0);
        ...
        mTextPaddingX = ta.getDimension(R.styleable.SlidingTabLayout_tl_textPaddingX, dp2px(4));
        mTextPaddingY = ta.getDimension(R.styleable.SlidingTabLayout_tl_textPaddingY, dp2px(2));
        mTextSpace = ta.getDimension(R.styleable.SlidingTabLayout_tl_textSpace, dp2px(0));
    ...
    private void addTab(final int position, String title, View tabView) {
        ...
        if (mTextSpace > 0) {
            lp_tab.leftMargin = (int) mTextSpace;
        }

        if (position == mTabCount - 1) {
            lp_tab.rightMargin = (int) mTextSpace;
        }
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            if (tv_tab_title != null) {
                ...
                tv_tab_title.setPadding((int) mTextPaddingX, (int) mTextPaddingY, (int) mTextPaddingX, (int) mTextPaddingY);
                if (mTextAllCaps) {
                ...
                if (isGradient && i == mCurrentTab) {
                    TextColorUtil.setTextViewStyles(tv_tab_title, Color.parseColor("#ff7f97"), Color.parseColor("#b776cb"), Color.parseColor("#6f6cff"));
                } else {
    ...
    private void scrollToCurrentTab() {
        ...
        int offset = (int) (mCurrentPositionOffset * (mTabsContainer.getChildAt(mCurrentTab).getWidth() + mTextSpace));
    ...
    public void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
                ...
                if (isGradient && isSelect) {
                    TextColorUtil.setTextViewStyles(tab_title, Color.parseColor("#ff7f97"), Color.parseColor("#b776cb"), Color.parseColor("#6f6cff"));
                } else {
                    tab_title.setTextColor(isSelect ? mTextSelectColor : mTextUnselectColor);
                    tab_title.getPaint().setShader(null);
                }
            }
        }
    }
    public void updateTabSelection(int position,boolean isGradient) {
        for (int i = 0; i < mTabCount; ++i) {
            View tabView = mTabsContainer.getChildAt(i);
            final boolean isSelect = i == position;
            TextView tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
