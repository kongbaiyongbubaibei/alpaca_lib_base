...
public class SelectDialog extends Dialog implements OnClickListener, OnItemClickListener {
    ...
    public SelectDialog(Activity activity, int theme, SelectDialogListener listener, List<String> names, String title, int firstItemColor) {
        super(activity, theme);
        mActivity = activity;
        mListener = listener;
        this.mName = names;
        mTitle = title;
        setItemColor(firstItemColor,firstItemColor);
        // 设置是否点击外围可解散
        setCanceledOnTouchOutside(true);
    }
